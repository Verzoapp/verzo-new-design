...
const DiscountBanner = () => {
    ...
    return (
        <div className={`text-sm md:text-base lg:text-xl bg-brandTint  text-lightBlue items-center h-9 text-center px-4 ${disountBadge ? "block" :"hidden"}`}>
            <div className='container flex h-full items-center mx-auto'>
                ...
                <div className='mr-5 md:mr-0'>
                    <Image src={CloseIcon} alt='icon' className='cursor-pointer h-6' onClick={() => closeDiscountBadge(false)}/>
                </div>
            </div>
        </div>
    )
